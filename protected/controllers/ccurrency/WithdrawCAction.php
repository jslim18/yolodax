...
class WithdrawCAction extends CAction{
	...
	$criteria->condition = "status in ('completed')";
	 ...
	 $data['pages'] = $pages;
		...
		$data['action_title'] = 'Completed';
		$data['action'] = Yii::app()->getBaseUrl(true)."/ccurrency/withdraw_complete";
		$data['url'] = 'ccurrency/withdraw_complete';
		$data['widreqs'] = Admins::model()
						->get_withdraw_request($offset, $limit, array('completed'));
		$data['type'] = "completed";
