...
<div style="font-size:11px; color:#FF0000; font-weight:normal; padding:0px;"><?php echo $form->error($user,'skype'); ?></div>
								...
								<tr>
									...
									<td>
<?php $st = $user['verify']!=0 ? "checked":"";
echo $form->checkBox($user,'verify',array("checked"=>$st)); ?>
<?php if($user['verify']==0){echo 'Not Verified!';}else{echo 'Verified!';}?>
									</td>
								</tr>
								<tr>
									<th bgcolor="#e3e3f3" align="right">Account Status?</th>
									<td>
<?php $st = $user['status']!=0 ? "checked":"";
echo $form->checkBox($user,'status',array("checked"=>$st)); ?>
<?php if($user['status']==0){echo 'Not Activated!';}else{echo 'Activated!';}?>
									</td>
								</tr>
								<tr>
									<th bgcolor="#e3e3f3" align="right">Google 2 Factor?</th>
									<td>
<?php $st = $user['is_g2a']!='no' ? "checked":"";
echo $form->checkBox($user,'is_g2a',array("checked"=>$st)); ?>
...
<?php if($user['status']==0){
	echo 'Not verified!';
}elseif(($user['status']==1 or $user['verify']==1) and ($user['is_g2a']==0 && $user['billing_address_status']==0 && $user['IC_status']==0)){
echo 'Pre-verified!';
}else{echo 'Verified!';}
...
?>
								...
								<tr>
									...
									<td>
<input type="password" class="input1 input2" size="40" placeholder="Password" value="<?php echo $user['confirm_password']?>" name="password" />
									</td>
								</tr>
								<tr>
								...
								<tr>
									<td bgcolor="#e5e5e5" align="center" colspan="3">
										<input class="button submit" type="submit" name="update" value="Save" />
										<input class="button cancel" type="button" name="update" value="Cancel" onclick="location.href='<?php echo Yii::app()->baseUrl.'/admin/users/?action=edit';?>'" />
									</td>
								</tr>
							</table>
<?php $this->endWidget(); ?>
